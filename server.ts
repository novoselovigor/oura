...
// Lazy initialization of GoogleGenAI to prevent crashing at boot if keys are missing
...
// Oura OAuth 2.0 Auth URL Generation Endpoint
app.get('/api/auth/url', (req, res) => {
  try {
    ...
    // Build standard Oura OAuth 2.0 Authorization URL
...
// Oura OAuth 2.0 Callback and Token Exchange Endpoint
app.get(['/auth/callback', '/auth/callback/'], async (req, res) => {
  try {
    ...
    // Determine absolute redirect URI matching what was registered
    ...
    // Render successful completion page that notifies the React dashboard popup listener
    res.send(`
      <!DOCTYPE html>
      <html>
        <head>
        ...
        <body>
          <div class="card">
            <h2>Аутентификация успешно завершена</h2>
            <p>Ваш токен доступа Oura успешно сгенерирован. Передаем ваши показатели на основную панель аналитики...</p>
...
// Periodic Sync Oura Telemetry Data Proxy
app.post('/api/sync-oura', async (req, res) => {
  try {
    ...
    // Compute start_date and end_date (last 30 days of trends)
    ...
    // Call sleep, daily_sleep, daily_activity, and daily_readiness collections concurrently
    ...
    // Handle initial network auth or rate limit errors
    ...
    // Map files by day string to merge
    ...
    // Generate unique sorted union of dates
    ...
    // Safe helper to convert seconds to minutes
...
app.post('/api/analyze-oura', async (req, res) => {
  try {
    ...
    // Prepare a concise summary of the metric series for Gemini context
...
// Chat with Oura AI Coach Endpoint
app.post('/api/chat', async (req, res) => {
  try {
    ...
    // Prepare a quick summary of the data context for the chatbot
...
`;
    ...
    // Reconstruct conversation messages for Gemini API chat
    // We will pass the conversation as a list of contents, ending with the user's latest prompt
    const apiMessages = messages.map((msg: any) => ({
      role: msg.role === 'assistant' ? 'model' : 'user',
      parts: [{ text: msg.content }]
    }));

    // If there is a system instruction, we pass it in the config
    const response = await ai.models.generateContent({
      model: 'gemini-3.5-flash',
...
// -------------------------------------------------------------
// Vite or Production Static Build Server Setup
// -------------------------------------------------------------
async function startServer() {
  if (process.env.NODE_ENV !== 'production') {
    // In development mode, wrap Express with the Vite dev server
    ...
    // In production mode, serve compiled static assets from /dist
